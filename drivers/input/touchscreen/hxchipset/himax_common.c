...
#if defined(HX_SMART_WAKEUP)
static int lct_hx_tp_gesture_callback(bool flag);
#endif
...
/*2020.4.26 longcheer taocheng add for gesture-feature start*/
#if defined(HX_SMART_WAKEUP)
#define WAKEUP_OFF 4
#define WAKEUP_ON 5
int hx_gesture_switch(struct input_dev *dev, unsigned int type, unsigned int code, int value)
{
	I("Enter. type = %u, code = %u, value = %d\n", type, code, value);
	if (type == EV_SYN && code == SYN_CONFIG) {
		if (value == WAKEUP_OFF)
			lct_hx_tp_gesture_callback(false);
		else if (value == WAKEUP_ON)
			lct_hx_tp_gesture_callback(true);
	}
	I("Exit\n");
	return 0;
}
#endif
/*2020.4.26 longcheer taocheng add for gesture-feature end*/
...
int lct_hx_tp_work_callback(bool en)
{
	if (en) {
		himax_int_enable(1);/* enable irq */
	} else {
	...
	I("%s Touchpad",en?"Enable":"Disable");
	return 0;
...
#if defined(HX_SMART_WAKEUP)
	ts->input_dev->event = hx_gesture_switch;
	ret = init_lct_tp_gesture(lct_hx_tp_gesture_callback);

	if (ret < 0) {
		E("init_lct_tp_gesture Failed!\n");
		goto err_init_lct_tp_gesture_failed;
	} else {
		I("init_lct_tp_gesture Succeeded!\n");
	}
#endif
...
#ifdef LCT_TP_WORK_EN
	err = init_lct_tp_work(lct_hx_tp_work_callback);
	if (err < 0) {
		E("init_lct_tp_work Failed!");
		goto err_init_lct_tp_work_fail;
	} else {
		I("init_lct_tp_work Succeeded!");
	}
#endif
...
*/
	...
	if (get_lct_tp_work_status())
		...
		E("Touchscreen Disabled, Can't enable irq!");
#else
	himax_int_enable(1);
#endif
...
/*2020.4.26 longcheer taocheng add for gesture-feature start*/
#if defined(HX_SMART_WAKEUP)
int lct_hx_tp_gesture_callback(bool flag)
{
	struct himax_ts_data *ts = private_ts;
	...
	if (flag) {
		...
		ts->gesture_cust_en[0] = 0;
			...
			tpd_gesture_flag = false;
#endif
		I("disable gesture mode\n");
	}

	I("%s: tp gesture state is %d\n", __func__, ts->SMWP_enable);
